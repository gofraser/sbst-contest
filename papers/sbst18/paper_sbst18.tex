...
etc.)~\cite{arcuri2014automated}.

Overall, no flaky tests were generated for the vast majority of
subjects in the competition. Amongst those benchmarks for which
...
types; and c) global property not being reset in between
executions. Further investigation will be needed to confirm these
...
% mocking
~\cite{ICST_Mocking17}. In this competition, a total of 2,766 mock
instances were created, 1,339 private fields were set and 2,409
private methods were invoked using mocked access. Some of the Java
classes most frequently used for mocking across benchmarks are
\texttt{File} (849 instances in total), \texttt{Throwable} (284),
\texttt{PrintStream} (210) and \texttt{Thread} (114). Mocking was
particularly useful for \EVOSUITE to generate tests for the FASTJSON
benchmarks, where multiple calendar systems are necessary (e.g.,
classes \texttt{HijrahDate}, \texttt{MinguoDate},
\texttt{JapaneseDate}, \texttt{ThaiBuddhistDate} were mocked 719 times
altogether).
...
incapable of generating any test. For the REDISSON benchmarks, most
